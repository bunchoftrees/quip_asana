...
ASANA_PROJECT_IDS = ['project_id_1', 'project_id_2', 'project_id_3']  # Add all your project IDs here
ASANA_HEADERS = {
        ...
        if due_date:
            ...
            for field in task['custom_fields']:
                if field['name'] == 'Blockers' and field['text_value']:
                    blockers.append(f"- {task['name']} (Blocker: {field['text_value']})")
...
def lambda_handler(event, context):
    start_date_last_week = (datetime.now() - timedelta(days=14)).strftime('%Y-%m-%d')
    end_date_next_week = (datetime.now() + timedelta(days=7)).strftime('%Y-%m-%d')
    ...
    for project_id in ASANA_PROJECT_IDS:
        tasks = get_asana_tasks(project_id, start_date_last_week, end_date_next_week)
        last_week_tasks, next_week_tasks, blockers = format_tasks_for_quip(tasks)
        all_last_week_tasks.extend(last_week_tasks)
        all_next_week_tasks.extend(next_week_tasks)
        all_blockers.extend(blockers)

    create_quip_document(all_last_week_tasks, all_next_week_tasks, all_blockers)
